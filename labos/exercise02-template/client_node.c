...
// conditional compilation option to control the number of measurements this
// sensor node wil generate
#if (LOOPS > 1)
#define UPDATE(i) (i--)
#else
#define LOOPS 1
...
/**
 * For starting the sensor node 3 command line arguments are needed. These
...
int main(int argc, char *argv[]) {
  ...
  // open TCP connection to the server; server is listening to SERVER_IP and
  // PORT
  ...
  while (i) {
    ...
    printf("Sending: %s\n", quotes[index]);
    if (tcp_send(client, (void *)&(quotes[index]), &bytes) != TCP_NO_ERROR)
      exit(EXIT_FAILURE);
    sleep(sleep_time);
    UPDATE(i);
  }
