...
#define TCP_DEBUG_PRINTF(condition, ...)                                       \
  do {                                                                         \
    if ((condition)) {                                                         \
      fprintf(stderr, "\nIn %s - function %s at line %d: ", __FILE__,          \
              __func__, __LINE__);                                             \
...
#define MAGIC_COOKIE                                                           \
  (long)(0xA2E1CF37D35) // used to check if a socket is bounded
 ...
 */
struct tcpsock {
  long cookie; /**< if the socket is bound, cookie should be equal to
                  MAGIC_COOKIE */
...
int tcp_passive_open(tcpsock_t **sock, int port) {
  ...
  TCP_ERR_HANDLER(((port < MIN_PORT) || (port > MAX_PORT)),
  ...
  TCP_DEBUG_PRINTF(s->sd < 0, "Socket() failed with errno = %d [%s]", errno,
                   strerror(errno));
  ...
  TCP_DEBUG_PRINTF(result == -1, "Bind() failed with errno = %d [%s]", errno,
                   strerror(errno));
  TCP_ERR_HANDLER(result != 0, free(s); return TCP_SOCKOP_ERROR);
  result = listen(s->sd, MAX_PENDING);
  TCP_DEBUG_PRINTF(result == -1, "Listen() failed with errno = %d [%s]", errno,
                   strerror(errno));
  TCP_ERR_HANDLER(result != 0, free(s); return TCP_SOCKOP_ERROR);
...
int tcp_active_open(tcpsock_t **sock, int remote_port, char *remote_ip) {
  ...
  TCP_ERR_HANDLER(((remote_port < MIN_PORT) || (remote_port > MAX_PORT)),
                  return TCP_ADDRESS_ERROR); // server port between 0 and
                                             // MIN_PORT is allowed
  ...
  TCP_DEBUG_PRINTF(client->sd < 0, "Socket() failed with errno = %d [%s]",
  ...
  TCP_DEBUG_PRINTF(result == -1, "Connect() failed with errno = %d [%s]", errno,
                   strerror(errno));
  TCP_ERR_HANDLER(result != 0, free(client); return TCP_SOCKOP_ERROR);
  memset(&addr, 0, sizeof(struct sockaddr_in));
  length = sizeof(addr);
  result =
      getsockname(client->sd, (struct sockaddr *)&addr, (socklen_t *)&length);
  TCP_DEBUG_PRINTF(result == -1, "getsockname() failed with errno = %d [%s]",
                   errno, strerror(errno));
  TCP_ERR_HANDLER(result != 0, free(client); return TCP_SOCKOP_ERROR);
...
int tcp_close(tcpsock_t **socket) {
  ...
  {
    if ((*socket)->ip_addr !=
        NULL) // then assume memory is allocated and must be freed
    {
      free((*socket)->ip_addr);
    }
    if ((*socket)->sd >= 0) {
      ...
      // if ((result of shutdown==-1)&&(errno!=ENOTCONN)) //socket wasn't
      // connected
      TCP_DEBUG_PRINTF(result == -1, "Shutdown() failed with errno = %d [%s]",
                       errno, strerror(errno));
      if (result != -1) {
        result = close((*socket)->sd); // try to close the socket descriptor
        TCP_DEBUG_PRINTF(result == -1, "Close() failed with errno = %d [%s]",
                         errno, strerror(errno));
      }
    }
  }
  // overwrite memory before free to make socket invalid (even if memory is
  // accidently reused)!
  (*socket)->cookie = 0;
  (*socket)->port = -1;
  (*socket)->sd = -1;
...
int tcp_wait_for_connection(tcpsock_t *socket, tcpsock_t **new_socket) {
  ...
  TCP_DEBUG_PRINTF(s->sd == -1, "Accept() failed with errno = %d [%s]", errno,
                   strerror(errno));
...
int tcp_send(tcpsock_t *socket, void *buffer, int *buf_size) {
  ...
  // if socket is not connected, a SIGPIPE signal is sent which terminates the
  ...
  *buf_size = sendto(socket->sd, (const void *)buffer, *buf_size, MSG_NOSIGNAL,
                     NULL, 0);
  TCP_DEBUG_PRINTF((*buf_size == 0), "Send() : no connection to peer\n");
  TCP_ERR_HANDLER(*buf_size == 0, return TCP_CONNECTION_CLOSED);
  TCP_DEBUG_PRINTF(
      ((*buf_size < 0) && ((errno == EPIPE) || (errno == ENOTCONN))),
      "Send() : no connection to peer\n");
  TCP_ERR_HANDLER(
      ((*buf_size < 0) && ((errno == EPIPE) || (errno == ENOTCONN))),
      return TCP_CONNECTION_CLOSED);
  TCP_DEBUG_PRINTF(*buf_size < 0, "Send() failed with errno = %d [%s]", errno,
                   strerror(errno));
...
int tcp_receive(tcpsock_t *socket, void *buffer, int *buf_size) {
  ...
  *buf_size = recv(socket->sd, buffer, *buf_size, 0);
  ...
  TCP_ERR_HANDLER(*buf_size == 0, return TCP_CONNECTION_CLOSED);
  TCP_DEBUG_PRINTF((*buf_size < 0) && (errno == ENOTCONN),
                   "Recv() : no connection to peer\n");
  TCP_ERR_HANDLER((*buf_size < 0) && (errno == ENOTCONN),
                  return TCP_CONNECTION_CLOSED);
  TCP_DEBUG_PRINTF(*buf_size < 0, "Recv() failed with errno = %d [%s]", errno,
                   strerror(errno));
  TCP_ERR_HANDLER(*buf_size < 0, return TCP_SOCKOP_ERROR);
  return TCP_NO_ERROR;
}
